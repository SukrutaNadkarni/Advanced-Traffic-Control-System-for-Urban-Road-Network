...
speeds = {'car': 4.5, 'bus': 3, 'truck': 2.25, 'bike': 4}  # average speeds of vehicles
...
weights={'car':2,'bus':4,'truck':6,'bike':1}
...
def countVehicles():
    ...
    print(f"Vehicle Counts: {counts} (Cars: {noOfCars}, Buses: {noOfBuses}, Trucks: {noOfTrucks}, Bikes: {noOfBikes})")
    weighted=[0,0,0,0]
    for i in range(4):
        weighted[i]=noOfCars[i]*weights['car']+noOfBikes[i]*weights['bike']+noOfBuses[i]*weights['bus']+noOfTrucks[i]*weights['truck']
    return counts, noOfCars, noOfBuses, noOfTrucks, noOfBikes,weighted
...
def repeat():
    ...
    while (True):
        counts, noOfCars, noOfBuses, noOfTrucks, noOfBikes,weighted = countVehicles()
