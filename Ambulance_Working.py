...
def repeat():
    ...
    while True:
        ...
        if ambulance_present:
            # Set the green signal to the ambulance's direction
            currentGreen = ambulance_direction
            # Set a reduced fixed green time for the ambulance (e.g., 5 seconds)
            signals[currentGreen].green = 5  # Reduced from 10 to 5 seconds
        else:
        ...
        # Check if the current green signal needs to stay green in the next cycle
        nextGreen = (currentGreen + 1) % noOfSignals
        if weighted[currentGreen] > weighted[nextGreen]:
            # If the current green signal still has more vehicles, extend the green signal
            signals[currentGreen].green = 5  # Extend by 5 seconds (adjust as needed)
            continue  # Skip the yellow and red transitions

        # If not, proceed with the normal yellow and red transitions
        # Dynamic yellow signal timing
        if counts[currentGreen] > 0:
            # Adjust yellow time dynamically based on the number of vehicles
            signals[currentGreen].yellow = max(1, min(3, counts[currentGreen] // 5))  # 1-3 seconds based on vehicle count
        else:
        ...
        signals[nextGreen].red = signals[currentGreen].yellow + signals[currentGreen].green  # set the red time of next to next signal as (yellow time + green time) of next signal  # set the red time of next to next signal as (yellow time + green time) of next signal  # set the red time of next to next signal as (yellow time + green time) of next signal
