...
def countVehicles():
            ...
            for vehicle in vehicles[direction][lane]:
                if not vehicle.crossed:
                    if hasattr(vehicle, 'image'):  # Check if the vehicle has the 'image' attribute
                        ...
                        if direction == 'right':
                            if (vehicle.x + rect.width > stopLines[direction] - 590 and vehicle.x + rect.width < stopLines[direction]):
                                counts[i] += 1
                            ...
                            if (vehicle.x < stopLines[direction] + 590 and vehicle.x > stopLines[direction]):
                                counts[i] += 1
                                if vehicle.vehicleClass == 'car':
                                    noOfCars[i] += 1
                                elif vehicle.vehicleClass == 'bus':
    ...
    return counts, noOfCars, noOfBuses, noOfTrucks, noOfBikes
